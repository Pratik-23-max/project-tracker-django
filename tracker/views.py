...
def index(request):
    # If a user lands on index while already logged in, send them to their dashboard
...
def dashboard(request):
    ...
    # We use filter to ensure a manager doesn't see other managers' projects
    projects = Project.objects.filter(manager=request.user)
    return render(request, 'tracker/dashboard.html', {'projects': projects})

@login_required
def project_detail(request, project_id):
    # Security check: Ensure the manager accessing this project is the one who owns it
    project = get_object_or_404(Project, id=project_id, manager=request.user)
    tasks = project.tasks.all() # Uses the 'related_name' from models.py
...
def employee_dashboard(request):
    ...
    # Added ordering so the newest tasks appear at the top
...
def update_task_status(request, task_id):
    ...
    if request.method == 'POST':
        ...
        # Validation to prevent invalid status injections
